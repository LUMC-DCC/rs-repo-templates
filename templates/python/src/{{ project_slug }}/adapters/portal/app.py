"""FastAPI portal application for {{ (project_name or project_slug) }}.
...
from {{ project_slug }}.adapters.portal.routes import index, records
...
def create_portal_app(settings: Settings | None = None) -> FastAPI:
    ...
    portal = FastAPI(
        title="{{ (project_name or project_slug) }} portal",
