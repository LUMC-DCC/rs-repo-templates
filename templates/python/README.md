...
{% if urls.homepage and urls.homepage != urls.repository %}
Homepage: {{ urls.homepage }}
{% endif %}
{% if urls.documentation and urls.documentation != urls.repository and urls.documentation != urls.homepage %}
...
{% set has_tests = test_types.entries | length > 0 %}
{% set has_local_checks = "CHANGELOG.md" in community_files.entries or has_quality_checks or has_tests %}
...
{% set has_runtime_configuration = has_http or security_measures.selected.entries | select("in", configuration_security_measures) | list | length > 0 %}
...
| Python library | Import `process_text` from `{{ project_slug }}`. |
{% endif %}
{% if "Command-line tool" in interface_types.values %}
| Command line | `{{ project_slug | replace('_', '-') }} process "example input"` |
{% endif %}
{% if "Script" in interface_types.values %}
| Script | `@@PROJECT_RUN@@python scripts/run_example.py "example input"` |
{% endif %}
{% if has_http %}
| HTTP interfaces | `@@PROJECT_RUN@@{{ project_slug | replace('_', '-') }}-serve` |
{% endif %}
{% if "Desktop application" in interface_types.values %}
| Desktop application | `@@PROJECT_RUN@@{{ project_slug | replace('_', '-') }}-desktop` |
{% endif %}
{% if "Plug-in" in interface_types.values %}
| Plug-in | Load entry points from `{{ project_slug }}.plugins`. |
{% endif %}
...
{% if documentation_types.entries %}
...
by `{{ project_slug }}.config.Settings`; keep secrets out of version
...
{% if has_local_checks or include_metadata %}
## Development
