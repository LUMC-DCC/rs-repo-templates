...
{% set effective_formatter_tool = quality_tools.formatter if quality_tools.formatter in template_supported_choices.quality_tools.formatter else "" %}
{% set effective_linter_tool = quality_tools.linter if quality_tools.linter in template_supported_choices.quality_tools.linter else "" %}
{% set effective_type_checker = quality_tools.type_checker if quality_tools.type_checker in template_supported_choices.quality_tools.type_checker else "" %}
...
{% set has_docs_workflow = documentation_types.entries and effective_documentation_builder in ["mkdocs", "zensical", "sphinx"] %}
{% set has_license_workflow = licensing.compatibility_check == "Yes - automated tooling" and licensing.license | trim %}
{% set has_security_workflow = "Vulnerability scanning (e.g., Snyk, Dependabot)" in security_measures.selected.entries %}
{% set container_types = namespace(values=[]) %}
{% for container in containerization.entries %}
...
{% set selected_distribution_channels = namespace(values=[]) %}
{% for channel in distribution_channels.entries %}
{% set _ = selected_distribution_channels.values.append(channel | lower) %}
{% endfor %}
{% set has_distribution_workflow = "pypi" in selected_distribution_channels.values or "github releases" in selected_distribution_channels.values or "github release" in selected_distribution_channels.values or "conda-forge" in selected_distribution_channels.values %}
{% set has_ci_workflow = include_metadata or test_types.entries or has_quality_checks or has_docs_workflow or "CHANGELOG.md" in community_files.entries or has_license_workflow or has_security_workflow or has_container_workflow or has_distribution_workflow %}
...
{{ (project_name or project_slug) }} uses a `src`
...
`src/{{ project_slug }}/__init__.py`; implementation belongs in
...
| Path | Responsibility |
| --- | --- |
| `src/{{ project_slug }}/` | Importable application package and public Python API. |
| `src/{{ project_slug }}/services/` | Behavior that is independent of a delivery interface. |
{% if has_adapter %}
| `src/{{ project_slug }}/adapters/` | Selected interfaces and their input/output translation. |
{% endif %}
...
| `src/{{ project_slug }}/workflows/` | Importable Python workflow orchestration. |
...
| `src/{{ project_slug }}/ontology/` | RDF graph construction, ontology metadata, validation, and serialization. |
{% endif %}
{% if has_runtime_configuration %}
| `.env.example`, `src/{{ project_slug }}/config.py` | Documented environment settings and their typed runtime boundary. |
{% endif %}
...
Python workflow orchestration lives in `src/{{ project_slug }}/workflows/`.
...
```bash
{% if "CHANGELOG.md" in community_files.entries %}
