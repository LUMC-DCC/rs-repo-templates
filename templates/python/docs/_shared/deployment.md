...
`{{ project_slug }}.config.Settings` validates environment and
...
```bash
python -m {{ project_slug }}.adapters.desktop.app
```
...
application that discovers the `{{ project_slug }}.plugins` entry
point group.
