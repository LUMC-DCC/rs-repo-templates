...
def uses_biotools(ctx):
    """Return whether bio.tools is a selected software registry.
    ...
    -------
    bool
        Whether bio.tools registry metadata should be generated.
    """
    return any(
        "".join(
            character
            for character in str(registry.get("name", "")).lower()
            if character.isalpha()
        )
        == "biotools"
        for registry in entries(ctx, "registries")
...
def selected_models(ctx, spdx_identifier=None):
    ...
    if uses_biotools(ctx):
        biotools_model = model_from_context(BiotoolsModel, ctx)
        # bio.tools accepts SPDX identifiers and a few controlled non-SPDX
        # values. Omit custom license text that cannot satisfy its schema.
        biotools_model.licensing.license = spdx_identifier or ""
        models.append(biotools_model)
    return models
