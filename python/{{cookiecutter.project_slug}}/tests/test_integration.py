...
def test_web_application_serves_a_browser_page():
    ...
    assert response.status_code == 200
    assert "{{ (cookiecutter.project_name or cookiecutter.project_slug) }}" in response.text
...
def test_combined_server_mounts_selected_http_interfaces():
    ...
    from {{ cookiecutter.project_slug }}.adapters.server import app
...
def test_suite_runner_dispatches_a_registered_command():
    ...
    from {{ cookiecutter.project_slug }}.adapters.suite.runner import (
        run_suite_command,
        suite_commands,
    )
