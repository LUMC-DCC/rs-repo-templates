...
def test_runtime_settings_read_project_environment(monkeypatch):
    ...
    monkeypatch.setenv("{{ cookiecutter.project_slug | upper }}_LOG_LEVEL", "DEBUG")
...
def test_logging_configuration_sets_package_level():
    ...
    assert package_logger.name == "{{ cookiecutter.project_slug }}"
    assert (
        logging.getLogger(
            "{{ cookiecutter.project_slug }}.services.processing"
        ).getEffectiveLevel()
        == logging.DEBUG
    )
{% endif %}
...
def test_http_server_settings_are_typed(monkeypatch):
    ...
    monkeypatch.setenv("{{ cookiecutter.project_slug | upper }}_SERVER_PORT", "9000")
    monkeypatch.setenv(
...
{% if has_openapi_interface %}
    monkeypatch.setenv(
        "{{ cookiecutter.project_slug | upper }}_PUBLIC_BASE_URL",
        "https://example.org/{{ cookiecutter.project_slug }}/",
    )
{% endif %}
    ...
    assert settings.server_root_path == "/services/{{ cookiecutter.project_slug }}"
{% if has_openapi_interface %}
