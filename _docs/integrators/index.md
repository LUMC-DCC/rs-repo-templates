...
The latter two packages are generator-only: generated repositories do not depend
on them. Maintenance and CI checks compare their schema and file-model APIs and
...
Call `validate_document()` before constructing generated Pydantic models. It checks
JSON Schema conditionals, including the requirement that a non-empty contributor
list credit an author.
