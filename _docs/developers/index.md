...
- controlled RSM choices supported by each repository scaffold;
- values exposed to Jinja and finalization as hidden computed answers.

The generated questions adapt RSM fields to Copier. Do not edit them directly.
Regenerate them after an RSM dependency or policy change:
...
Avoid user-specific state, timestamps, and unbounded side effects. Use the network
only for required external data, such as SPDX license text, and fail generation if
