...
and `quality.yml`. The RSM controlled vocabulary has no separate R static
type-checker option.
...
A bio.tools registry selection generates `biotools.json` from the same RSM fields
used by the other scaffolds.
...
The same RSM fields supply access terms, funding identifiers, code-review policy,
sustainability, security, data management, functions, interfaces, external
dependencies, and services across README, documentation, and metadata.
