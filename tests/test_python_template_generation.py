...
def test_python_biotools_registry_generates_registry_metadata(
    tmp_path,
    monkeypatch,
):
    """Ensure a bio.tools registry entry writes schema-shaped metadata."""
    project_path = render_python_project(tmp_path, monkeypatch)

    metadata = json.loads((project_path / "biotools.json").read_text())
    assert len(metadata) == 1
    tool = metadata[0]
    assert tool["name"] == "Research Template Demo"
    assert tool["homepage"] == "https://example.org/research-template-demo"
    assert tool["biotoolsID"] == "research-template-demo"
    assert tool["version"] == ["0.2.0"]
    assert tool["toolType"]
    assert tool["topic"] == [
        {"term": "Proteomics", "uri": "http://edamontology.org/topic_0121"}
    ]
    assert tool["language"] == ["Python", "R"]
    assert tool["license"] == "MIT"
    assert tool["cost"] == "Free of charge (with restrictions)"
    assert tool["function"][0]["operation"]
    assert tool["link"][0] == {
        "url": "https://github.com/LUMC-DCC/research-template-demo",
        "type": ["Repository"],
    }
...
def test_project_context_renders_in_python_readme_docs_and_metadata(
    ...
    for content in (readme, docs_overview):
        ...
        assert "[Proteomics](http://edamontology.org/topic_0121)" in content
    ...
    assert "## Research Topics" in readme
    assert "## Research topics" in docs_overview
    ...
    assert "**Topics**" not in docs_functions
